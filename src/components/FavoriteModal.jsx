...
function FavoriteModal({
  ...
  const [rating, setRating] = useState(existingFavorite?.rating || 1);
  ...
  function handleSubmit(event) {
    ...
    if (note.length > 200)
      nextErrors.note = 'The note cannot exceed 200 characters.';

    setErrors(nextErrors);
    if (Object.keys(nextErrors).length > 0) return;

    onSave({ rating: numericRating, tag: tag.trim(), note: note.trim() });
  }
    ...
    <div className="favorite-modal-backdrop" onMouseDown={onClose}>
      ...
      >
        ...
        <div className="favorite-modal-heading">
          ...
          <div>
            ...
            <h2 id="favorite-modal-title">
              {existingFavorite ? 'Edit' : 'Add'}{' '}
              {formatPokemonName(pokemon.name)}
            </h2>
          </div>
          {appearance.includes('shiny') && (
            <span className="favorite-shiny-tag">✦ SHINY</span>
          )}
        </div>
        ...
        <form className="favorite-form" onSubmit={handleSubmit} noValidate>
          ...
          />
          {errors.tag && (
            <p className="field-error" id="tag-error">
              {errors.tag}
            </p>
          )}
          ...
          />
          {errors.note && (
            <p className="field-error" id="note-error">
              {errors.note}
            </p>
          )}
